...
/**
 * Decision OS — Phase 7.28 @allfantasy/widget-js package verification.
 *
 ...
 * the package.json artifact (React as a REAL dependency, same strategy as
 * widget-web-component, per PHASE_7_22_SDK_PACKAGING_ADR.md D6), the
 * single-entry-point build config, and the BUILT `dist/` output — mirroring
 * the same checks Phase 7.23-7.27 already established for the five sibling
 * packages.
 */

const PACKAGE_ROOT = resolve(process.cwd(), 'sdk-runtime/js-embed')
const DIST_DIR = join(PACKAGE_ROOT, 'dist')
const DIST_INDEX_DTS = join(DIST_DIR, 'sdk-runtime/js-embed/src/index.d.ts')
...
describe('@allfantasy/widget-js — package.json', () => {
  it('is private:true — the technical guardrail against accidental npm publish', () => {
    const pkg = JSON.parse(readFileSync(join(PACKAGE_ROOT, 'package.json'), 'utf8'))
    expect(pkg.private).toBe(true)
  })

  it('declares sideEffects:false — attachAllFantasyGlobal is caller-invoked, never a module-load effect', () => {
    const pkg = JSON.parse(readFileSync(join(PACKAGE_ROOT, 'package.json'), 'utf8'))
  ...
  it('declares react and react-dom as REAL dependencies, not peers — same strategy as widget-web-component (ADR D6)', () => {
    const pkg = JSON.parse(readFileSync(join(PACKAGE_ROOT, 'package.json'), 'utf8'))
  ...
  it('has no dependency on any other embed-adapter package (widget-iframe, widget-web-component)', () => {
    const pkg = JSON.parse(readFileSync(join(PACKAGE_ROOT, 'package.json'), 'utf8'))
  ...
  it('the package name matches the ticket-requested @allfantasy/widget-js', () => {
    const pkg = JSON.parse(readFileSync(join(PACKAGE_ROOT, 'package.json'), 'utf8'))
    expect(pkg.name).toBe('@allfantasy/widget-js')
  })
})

describe('@allfantasy/widget-js — build config', () => {
  it('tsconfig.build.json is a NEW file — the pre-existing tsconfig.json (Phase 7.17) is untouched', () => {
...
// sdk-runtime/js-embed/ — SKIPS (not fails) if dist/ does not exist yet.

const distExists = existsSync(DIST_DIR)

describe.skipIf(!distExists)('@allfantasy/widget-js — build output (dist/)', () => {
  ...
  it('never references an iframe-adapter or web-component-specific concept anywhere in the built .d.ts tree', () => {
    // Deliberately does NOT forbid 'createAllFantasyWidget' or 'attachAllFantasyGlobal' —
    // those are THIS package's own legitimate exports (see the spot-check test above).
    // widget-iframe's own package-boundary test already documents this exact naming
    // collision from the other side (word-boundary check needed there, not here).
  ...
  it('known limitation (matches Phase 7.23-7.27): the transitive closure still mirrors lib/decision-os/behavioral/api/contracts.d.ts, sdk-runtime/core, and sdk-runtime/react into dist/ — content-safety re-confirmed here', () => {
    const files = walkFiles(DIST_DIR).filter((f) => f.endsWith('.d.ts'))
    ...
    // Phase 7.24/7.25/7.26/7.27 documented from the other sides.
