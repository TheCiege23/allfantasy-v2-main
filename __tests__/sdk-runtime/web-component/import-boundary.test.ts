...
/**
 * Architecture regression suite for `sdk-runtime/web-component`.
 *
 ...
 * decision D2) that `sdk-runtime/iframe/src/reactChild` (Phase 7.15)
 * established — it deliberately composes `sdk-runtime/react`. What must
 * still hold:
 ...
 *   - forbidden: `sdk-runtime/iframe` (a PEER adapter — proves adapter
 *     independence still holds except for the one sanctioned react import),
 *     lib/decision-os/behavioral/* (Phase 5/6 internals),
...
const WEB_COMPONENT_FILES = [
  ...
  'sdk-runtime/web-component/src/shadowMount.ts',
  'sdk-runtime/web-component/src/defaults.ts',
...
describe('architecture: sdk-runtime/web-component import boundary', () => {
  ...
  it('the allowlist guard actually catches a disallowed import (positive control)', () => {
    ...
    const allowedLocal = "import type { ParsedElementAttributes } from './attributes'"
  ...
  it('the sdk-runtime/react import is actually present — the sanctioned exception is exercised, not just theoretically allowed', () => {
    const allSpecifiers = WEB_COMPONENT_FILES.flatMap(([, src]) => extractImportSpecifiers(src))
    expect(allSpecifiers.some((s) => s.startsWith('../../react/src'))).toBe(true)
  })

  it('no file imports sdk-runtime/iframe (a peer adapter — proves independence still holds except the one sanctioned react import)', () => {
    for (const [path, src] of WEB_COMPONENT_FILES) {
  ...
  it('no file calls window.postMessage or references postMessage at all (this embed target uses CustomEvent, never postMessage)', () => {
    for (const [path, src] of WEB_COMPONENT_FILES) {
...
describe('architecture: sdk-runtime/web-component computes no intelligence', () => {
...
describe('architecture: sdk-runtime/web-component contains no internal Decision OS terminology', () => {
  const stripImportLines = (src: string) => src.replace(/^import\s[\s\S]*?from\s+['"][^'"]+['"]\s*$/gm, '')
  const WEB_COMPONENT_FILES_NO_IMPORTS = WEB_COMPONENT_FILES.map(([path, src]) => [path, stripImportLines(src)] as const)

  it('no source file leaks internal terminology outside of import paths', () => {
    for (const [path, src] of WEB_COMPONENT_FILES_NO_IMPORTS) {
  ...
  it('the denylist guard actually catches a violation (positive control)', () => {
    expect(hasInternalLeakage('This element exposes Decision OS internals')).toBe(true)
  })
})
