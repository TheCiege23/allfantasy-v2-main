...
vi.mock('@/lib/trade-intel/marketValueService', () => ({
  ...
  playerValue: (values: { bySleeperId: Record<string, { value: number }> }, id: string) =>
    values.bySleeperId[id]?.value ?? null,
}))
...
const VALUES = {
  ...
  ].map(([id, value]) => [id, { name: String(id), sleeperId: String(id), position: null, value, overallRank: null, trend30Day: null }])),
  byPick: {},
}
