...
beforeAll(() => {
  ...
  throw new Error(
    [
      ...
      '',
      '  To run them, confirm the target first and then opt in — and NAME the',
      '  target explicitly. vitest.setup.db-guard.ts pins DATABASE_URL shut when',
      '  you do not, so omitting it now refuses to connect rather than quietly',
      '  reaching production the way it used to:',
      '',
      '    npx tsx scripts/check-staging-env.ts     # exit 1 = not safe',
      `    ${OPT_IN}=1 DATABASE_URL=<non-prod> DIRECT_URL=<non-prod> \\`,
