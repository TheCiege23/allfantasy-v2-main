 ...
 *
 * WHY THIS EXISTS
 * 2026-09-06: the "one session batches and pushes" convention (CLAUDE.md) was
 * retired as a default now that production bills like Railway usage, not
 * Vercel build-minutes — see the dated section in CLAUDE.md for the numbers.
 * That convention's OTHER job, besides saving build minutes, was a human
 * running a fast typecheck over the union of everyone's pending commits right
 * before they landed. That is what caught the `LivePageData.fetchedAt` type
 * widening documented earlier in CLAUDE.md — a check scoped to "my files"
 * would have missed it, because the break showed up in a CONSUMER's file.
 *
 * Decentralizing pushes removes that reviewer. This is the automated stand-in:
 * every push to `main` gets its exact SHA typechecked in isolation before it
 * is allowed to land, using the same per-file ratchet
 * (`scripts/ts-error-ratchet.mjs`) the repo already trusts for this job.
 *
