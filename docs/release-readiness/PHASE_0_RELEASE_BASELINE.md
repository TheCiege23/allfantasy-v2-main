...
| 21 | Stripe checkout (monthly+annual) | **PRODUCTION READY — pricing verified 2026-09-05** | Charge is a server Checkout Session built from `STRIPE_PRICE_AF_*` (`lib/monetization/StripeCheckoutSession.ts:62,93` — `line_items:[{price: priceId}]`), **not** a payment link. All 11 SKUs: `catalog.ts` `amountUsd` == the Stripe `unit_amount` of the price its env var names (live API). ⚠ `STRIPE_CHECKOUT_LINK_AF_*` survives only behind `/api/admin/monetization/checkout-link-mapping`, which reports `issue:null` even for a deactivated link — do not read it as link health |
...
| Displayed subscription prices | pricing pages via `catalog.ts` | **RESOLVED 2026-09-05** — the payment link is not the charge path; a Checkout Session from `STRIPE_PRICE_AF_*` is | **VERIFIED** — 11/11 SKUs, `amountUsd` == Stripe `unit_amount` (live API) |
...
| B3 | ~~S2~~ **CLOSED 2026-09-05** | Pricing display-vs-charge drift — checkout may charge OLD prices | Payments / revenue | **The prescribed fix had already shipped.** `app/api/monetization/checkout/subscription/route.ts:101` → `buildStripeCheckoutSessionForSku` → `lib/monetization/StripeCheckoutSession.ts:62,93`, which charges `line_items:[{price: STRIPE_PRICE_AF_*}]` and fails soft (returns null, no charge) when the id is unset. ⚠ The evidence path cited here, `checkout/subscription/route.ts:99`, **does not exist** | was: two unreconciled price sources | done — server Checkout Sessions from catalog price IDs | **PASSED 2026-09-05** — all 11 SKUs, `catalog.ts` `amountUsd` == Stripe `unit_amount` via live API: Pro 9.99/79.99, Commissioner 14.99/129.99, Legacy 9.99/79.99, Supreme 19.99/159.99, tokens 4.99/8.99/19.99 | — |
...
| B7 | ~~S3~~ **CLOSED 2026-09-05** (landed `b4fdf849f`) | Imported league not auto-selected on dashboard; "Go to dashboard" drops the league id | Post-import | `LegacyImportResults.tsx:121` `goDashboard()` pushed only `?rankSync=1` | producer and consumer disagreed on the param NAME | 🛑 **NOT the fix written here.** `?leagueId=` is read by NOTHING — `/dashboard` is retired and redirects to `/core`, which reads `sp.league` (`app/core/[[...screen]]/page.tsx:366`); `lib/dashboard/dashboard-league-selection.ts` expects `leagueId` and has ZERO callers. Appending `leagueId` would have satisfied this row and changed nothing a user sees. Fixed by sending `league`, omitted entirely for the career-import variant that has no league | `__tests__/import-dashboard-league-param.test.ts` — asserts producer and consumer agree on the name; mutation-controlled (reverting to `leagueId` turns it red) | S |
...
| B11 | **S4 — MOSTLY RESOLVED 2026-09-05, one piece open** | Durable Sleeper sync shipped OFF; async drain unmerged | Data freshness | ⚠ Both evidence items are stale. `FANTASY_OS_EXEC_SYNC_LIVE` is **`"true"`** in Railway production, and **#347 is CLOSED UNMERGED**, not open | flag was off; the drain was never landed | **The closing test now passes on production, measured 2026-09-05 22:53Z against `ep-curly-block-ad0dlt9o`:** *cron runs* — `/api/cron/fantasy-os-exec-sync` at `*/10`; *freshness advances* — of 215 leagues in `league_sync_state`, 25 synced OK in the last 15 min, 156 in the last hour, 213 in 24h, newest success 7.5 min old; `LeagueTrade` 18,147 → 18,288 the same day; *no provider hammering* — **212 of 215 leagues carry ZERO consecutive failures**. **STILL OPEN:** the async drain (`sleeper-refresh-drain`) does not exist on `main` — #347 closed without merging, so that half was never delivered | ⚠ **AND ONE DEFECT THIS MEASUREMENT FOUND — RE-MEASURED 2026-09-06, AND THE NUMBERS THAT JUSTIFIED IT WERE WRONG BY TWO ORDERS OF MAGNITUDE.** This cell previously read: *"3 leagues permanently failing `League not found` with 92, 70 and 3 consecutive failures — retried forever at 10-min cadence, ~40 requests × 3 attempts each lap."* 🛑 **TWO OF THE THREE ARE ORPHANS.** Their `leagues` mirror row is gone, and `enumerateConnectedLeagues` selects FROM `leagues` — so they are never enumerated, never retried, and cost **zero** provider requests; `lastAttemptedSyncAt` is frozen at **2026-08-24** for both. They carried **162 of the 165 failures**, so 98% of the figure behind this item was dead residue, and nothing will ever reset it because nothing will ever run it. 🛑 **NEVER QUOTE A FAILURE COUNT FROM `league_sync_state` WITHOUT JOINING TO `leagues`** on `(platform=provider, platformLeagueId=externalLeagueId, season)` — orphans carry the *largest* counts precisely because they failed longest before being disconnected, so the table reads like a live-cost measure and is not one. **Exactly ONE league is genuinely failing** (`sleeper:1395068198813978624:2026`, healthy through 09-05 16:48): **15 runs in 24h totalling 79 seconds**, 3 attempts on `league_state` then abort — not ~40 requests, and ~90 min apart, not 10. The portfolio around it is healthy: **213 of 213** leagues synced in 24h. The B12 breaker does **not** catch this and must not: a 404 is a healthy answer. ⚠ **THE GIVE-UP IS BUILT BUT UNLANDED** — `2233065c1` on `fix/sleeper-retire-not-found-leagues` (`lib/import-os/retirement.ts` plus an owner-facing surface; 9 files, 860 insertions, with tests). It gates on *enumerated* leagues, so it will **not** clear the two orphan rows; that cleanup is a separate decision and deliberately not part of it | M |
| B12 | **S4 — PARTIALLY ADDRESSED 2026-09-05** (`0fba421d5`), **NOT closed** | OAuth providers unverifiable; provider budget/circuit-breaker absent for fantasy providers | Auth / reliability | ⚠ Most of this row was already false when checked: `lib/sports-router.ts` runs a real breaker over its own sources; `lib/workers/rate-limit-manager.ts` already DECLARES budgets for sleeper/yahoo/espn/mfl/fantrax; `SleeperLeagueFetchService` already bounds concurrency (`pLimit(10)`) with backoff | the genuine gap: nothing stopped calls being MADE when a provider refuses — concurrency caps in-flight, backoff spaces them, neither reduces the total. ~40 requests/league × 3 retries × a 235-league rotation that moved 30min→10min = ~28k requests per lap at a host already saying no | **DONE:** `lib/providers/provider-circuit.ts` wired at `fetchSleeperJson`, the one Sleeper ingestion chokepoint. **STILL OPEN — do not mark closed:** (1) the declared budgets are still UNENFORCED (needs a design call: per-request DB round-trip vs shared counter); (2) only Sleeper is wired — Yahoo/ESPN/MFL/Fantrax have no equivalent chokepoint; (3) `sports-router.ts` keeps a duplicate breaker worth migrating to the shared primitive; (4) `DISCORD_CLIENT_ID` is `[SENSITIVE]`-corrupt in Railway, the one provider whose creds genuinely are not set. ⚠ "Confirm creds in Vercel" is stale — production is Railway; Google/Facebook/Yahoo/Spotify are set | `__tests__/providers/provider-circuit.test.ts` — 13/13, mutation-controlled on both the 404-must-not-count rule and the wiring itself | M–L |
...
- **Env prerequisites:** `INVITE_ONLY`; OAuth `*_CLIENT_ID/SECRET`; `RESEND_API_KEY`/`RESEND_FROM`; `STRIPE_PRICE_AF_*` (the actual charge path — `STRIPE_CHECKOUT_LINK_AF_*` is **not** used for checkout); `FANTASY_OS_EXEC_SYNC_LIVE` (only after load test); `SENTRY_DSN`; Twilio/VAPID as needed; notification placeholder disabled.
...
- **Revenue:** subscriptions + tokens with **B3 pricing verified ✅ 2026-09-05** and **B14 no-charge-on-failure spot-audited**.
- **Must-fix before any invite:** B1 (data deletion), B2 (secret scanning/push-protection + purge request), B3 (pricing — **CLOSED 2026-09-05**), B4 (beta gate reconciled), B8 (notification placeholder off), B9 (token claim).
...
4. ~~Blocker B3~~ **DONE 2026-09-05.** Reconciliation complete, and it inverted the premise: the payment links are not the buyer path at all. `MonetizationPurchaseSurface.tsx` POSTs to `/api/monetization/checkout/subscription`, which builds a server Checkout Session from `STRIPE_PRICE_AF_*`. Verified against the live Stripe API — 11/11 SKUs charge the catalog `amountUsd`. No live-mode charge was made; prices were read, not exercised.
