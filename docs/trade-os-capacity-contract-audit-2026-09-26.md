...
Cap growth now uses a declared `League.settings.capStartYear` when available, otherwise the persisted configuration creation year, instead of resetting its origin each January. A historical import or a configuration created before its first playing season should declare the startup season explicitly. Future-cap previews read existing ledgers without writing, honor the league season, and exclude expired contracts from each year's count.
...
2. **Expose and capture the startup season during configuration/import.** The calculator now supports a stable declared origin; add commissioner UI and importer capture for historical leagues and leagues configured before their first playing season. Configuration creation year is a fallback, not evidence of an imported league's historical startup season.
