...
- The league-scoped notification browser audit found Mark all read sent the account-wide action. Added an optional validated league scope to the existing receipt API, always combined with the signed-in user filter. The selected-league screen sends that scope, including when older receipts are outside its loaded window. All 13 grouping, unread-count and API-scope checks passed across three files.
