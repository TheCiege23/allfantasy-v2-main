...
**Status:** Audit complete. **Phase A (Truth & Safety) implemented** — see the Phase A
status block below and the inline ✅ PHASE A markers.
...
## Phase A status — Truth, Safety, and Honest Availability

Phase A deliberately fixed only what was **untrue or unsafe**. It expanded no provider
capability, so **no provider's certification status improved** — messaging became honest,
which is not the same as a provider becoming more capable.

| Audit finding | Phase A outcome |
|---|---|
| Fleaflicker fabricates playoff count, roster size 40, description-as-scoring | ✅ Fixed — all three now report unavailable |
| Yahoo `playoffSettings` claims `full` without a playoff-team count | ✅ Fixed — never `full` unless playoffs are known-off |
| Yahoo `usesPlayoff: null` silently becomes `0` playoff teams | ✅ Fixed — only an explicit `false` yields `0` |
| Fantrax gate reason stale (`appUserId is never stamped`) | ✅ Corrected — claim disproven, real limitation recorded |
| `/api/auth/mfl` unauthenticated, plaintext, disconnected from importer | ✅ Gated — auth required, accepts nothing, data preserved |
| MFL UI collects a password the importer cannot use | ✅ Removed — form and misleading help modal deleted |
...
- **Cross-provider Legacy evidence bridge** — Phase B. Still `provider === 'sleeper'`.
- **Durable backfill queue** — Phase C. Phase A makes the *state* honest, not the delivery
  guaranteed. `historicalBackfillDurable` is `false` on every path today.
- **MFL API-key entry UI** — Phase D. `POST /api/league/auth` already accepts and encrypts
  `{ platform: 'mfl', apiKey }`; only the UI is missing. `MFLConnection` still lacks user
  linkage (schema change, deliberately not attempted here).
- **Standings `rank ?? teams.length` fallback** (ESPN/Yahoo/MFL) — same class of fabrication
  as the ones fixed here: a rank-less team is silently placed last rather than left unknown.
  Not fixed in Phase A because `NormalizedStandingsEntry.rank` is non-optional and champion
  /runner-up derivation in `LeagueSeason` sorts on it, so a correct fix is a type-contract
  change plus downstream rework — too large for a truth-and-safety PR, and risky to
  `LeagueSeason` champion data. **Tracked as the first Phase B truth item.**
- **Transactions still have no destination table** — Phase B.
...
- **Yahoo playoff settings ❌** — `playoff_team_count: raw.settings?.usesPlayoff ? undefined : 0`
  ...
  ✅ **PHASE A** — both halves fixed. The falsy test also meant `usesPlayoff: null` (Yahoo
  not reporting the setting) collapsed to a hard `0`, which *overrode* the documented
  playoff default downstream; only an explicit `false` now yields `0`. Coverage is `partial`
  when playoffs are on/unknown, `full` only when playoffs are explicitly disabled, `missing`
  when there are no settings. `faab_budget: null` was left alone — `null` is "unknown", not
  a fabrication, and the commit layer maps it to a no-op write.
...
| Line | Fabrication | Status |
|---|---|---|
| `:103` | `playoff_team_count: Math.max(2, Math.floor(leagueSize / 2))` — invented, not sourced. Coverage still claims `playoffSettings: 'partial'` rather than `missing`. | ✅ **PHASE A** — field omitted; coverage now `missing` |
| `:42` | `rosterSize: lg.rosterRequirements?.rosterSize ?? 40` — magic default 40. | ✅ **PHASE A** — now `?? null` |
| `:98` | `scoring: lg.description ?? 'imported'` — wrong field entirely. | ✅ **PHASE A** — now `null` |

All three are locked by behavioral invariants in
`__tests__/league-import/provider-truth-invariants.test.ts`, which also asserts a genuine
source-provided `rosterSize` still survives — the fix removed the fabrication, not the data.
...
✅ **PHASE A — state made honest; delivery still not guaranteed.** A durable queue is Phase
C and was explicitly not attempted. What changed, via the new pure module
`lib/league-import/historicalBackfillState.ts`:

- **Fleaflicker no longer claims a backfill.** It has no backfill service, so
  `runHistoricalBackfill` returned `null` — which the `.then` handler then recorded as
  `'complete'`, reporting a finished multi-season backfill that never ran. Unsupported
  providers are now stamped `'unsupported'` and are not dispatched at all.
- **`historicalBackfillDurable: false`** is recorded on every dispatch, so a reader can
  distinguish best-effort in-process work from durably-accepted work. Nothing sets it `true`
  yet — that is the honest state of the system.
- **`historicalBackfillStaleAfter`** bounds the claim. `resolveHistoricalBackfillState()`
  reports a `pending` record past its deadline as `'stale'`, so a lost dispatch surfaces as
  recoverable instead of eternally in-progress. Legacy rows with no deadline fall back to
  `startedAt` + the same window, so existing data converges without a migration.
- **A synchronous dispatch throw is now recorded** as `'failed'` with the real error, rather
  than becoming an unhandled rejection that left the league stamped `'pending'`.

`'pending'` was retained as the on-disk literal because `HistoryTab` and
`POST /api/leagues/{id}/backfill/retry` already key on it; staleness is layered on top
rather than renaming a value the existing retry path depends on. `GET /api/leagues/{id}/history`
now returns the **derived** state.
...
> ⚠ **PHASE A CORRECTION — this finding was partly overstated.** Phase 1 concluded that "no
> verified flow writes the credential the importer expects." That is **wrong**.
> `POST /api/league/auth` (`app/api/league/auth/route.ts`) lists `mfl` in
> `SUPPORTED_PLATFORMS`, requires a session, enforces an origin check, requires `apiKey` for
> MFL, `encrypt()`s it, and upserts the exact `LeagueAuth` row keyed `(userId, platform)`
> that `getDecryptedAuth(userId,'mfl')` reads. **The secure server-side path already exists
> and is correct.** What is missing is only a **UI** that calls it with an MFL API key — the
> app's only MFL credential screen posted to the broken `/api/auth/mfl` instead.
>
> ✅ **PHASE A ACTIONS.** `/api/auth/mfl` now requires an authenticated session and returns
> `503` with code `mfl_connection_unavailable` **before reading the request body** — it
> accepts, forwards, logs, and stores nothing. The `/af-legacy` username/password form and
> its help modal (which instructed users to enter a password and falsely claimed "your
> credentials are never stored") were removed. **No `MFLConnection` row or column was
> touched** — the model and all existing data are preserved for a future migration that adds
> user linkage. MFL remains `available: false` with reason `credential-entry-missing`.
...
✅ **PHASE A** — re-verified against `origin/main` and confirmed: the stamping claim is
false, the ownership enforcement is real on both the write side and the read side. The stale
comment was removed and replaced with the actual limitation, plus a machine-readable
`unavailableReason: 'manual-upload-required'`. The import wizard no longer says "coming
soon" for Fantrax — that framing implied unbuilt work, when the pipeline exists and only the
entry point is missing. A regression test asserts the disproven ownership explanation cannot
return.
