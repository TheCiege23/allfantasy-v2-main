...
- **Needs your action** (3-column grid). Card kinds: *Offer to you* (Accept / Reject / Load into builder), *On Sleeper · read-only* ("Act on it in Sleeper" / Load into builder — Sleeper has no write API), *Commissioner review* (Approve / Veto, with the veto vote count). Each shows both sides, projected grade tiles and a one-line read.
- **Your trades**: Active / Completed tabs. Rows: direction pill (INCOMING / OUTGOING / DONE) + when, YOU SEND, YOU GET · PARTNER, your grade tile with note, status chip. Completed rows carry **realized** grades; a withheld grade renders "—" with the reason (FAAB only, pick unpriced).
