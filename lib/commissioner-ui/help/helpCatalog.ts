...
/**
 * The Help & Knowledge Center catalog: authored prose about this product, shared by every data
 * mode.
 *
 * 🛑 IT USED TO LIVE INSIDE `demo.ts`, WHICH MEANT LIVE MODE HAD NO HELP AT ALL. The live client
 * returned `upstream_unavailable` on principle — the reasoning was that `source: 'live'` should mean
 * one consistent thing across all twelve namespaces, and Help's unusually static content should not
 * get to redefine it. The principle is sound and the outcome was not: a paying commissioner opened
 * Help and got an error, while the twelve articles explaining the product they had just bought sat
 * in a file only Demo Mode could reach.
 *
 * Help is genuinely different from the other eleven namespaces, and the honest way to say so is
 * `CATALOG_REVISED_AT` below rather than an error. There is no upstream to be unavailable — this
 * content ships with the deployment.
 */

/**
 * When this catalog was last authored.
 *
 * ⚠ NOT `new Date()`, WHICH IS WHAT IT USED TO BE. Every read stamped every article as updated
 * at that instant, so prose untouched for months claimed to be seconds old — the one field a reader
 * uses to judge whether documentation can be trusted. A constant is a fact; `now()` was a
 * fabrication shaped like one. Bump it when the prose changes.
 */
export const CATALOG_REVISED_AT = '2026-09-09T00:00:00.000Z'

function ts() {
  return CATALOG_REVISED_AT
}
 ...
 */
export const HELP_ARTICLES: CommissionerHelpArticleContract[] = [
  {
    id: 'help-welcome',
...
export const HELP_GLOSSARY: CommissionerGlossaryTermContract[] = [
  {
