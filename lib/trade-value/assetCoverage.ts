import type { AssetValueSnapshot, TradeAssetCoverage, TradeAssetCoverageItem } from './types'
...
export function assessTradeAssetCoverage(assets: AssetValueSnapshot[]): TradeAssetCoverage {
  ...
  return {
    ...
    totalCount,
    items,
    warnings: items.filter((i) => !i.resolved && i.reason).map((i) => i.reason as string),
  }
}
