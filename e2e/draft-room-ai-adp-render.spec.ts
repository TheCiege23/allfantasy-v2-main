...
const AI_ADP_POOL_ENTRIES = [
  makeEntry({ id: 'p-bijan', name: 'Bijan Robinson', position: 'RB', team: 'ATL', adp: 36.72, aiAdp: 3, aiAdpSampleSize: 12 }),
  makeEntry({ id: 'p-mike-evans', name: 'Mike Evans', position: 'WR', team: 'TB', adp: 31.57, aiAdp: 3.5, aiAdpSampleSize: 8, aiAdpLowSample: true }),
  makeEntry({ id: 'p-tj-hockenson', name: 'T.J. Hockenson', position: 'TE', team: 'MIN', adp: 93.61, aiAdp: 78, aiAdpSampleSize: 5, aiAdpLowSample: true }),
  makeEntry({ id: 'p-no-adp', name: 'Depth Player', position: 'WR', team: null, adp: null, aiAdp: null }),
]

test('draft room renders AI ADP column from pool API response', async ({ page }) => {
  page.setDefaultTimeout(15_000)
  page.setDefaultNavigationTimeout(45_000)
  ...
  await page.route('**/api/leagues/*/draft/pool**', async (route) => {
    await route.fulfill({
      ...
      body: JSON.stringify({
        entries: AI_ADP_POOL_ENTRIES,
        sport: 'NFL',
        count: AI_ADP_POOL_ENTRIES.length,
      }),
    })
  })
  ...
  // API → client state → normalizeDraftPlayer → cell.
