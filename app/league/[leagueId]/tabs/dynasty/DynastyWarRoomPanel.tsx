...
export function DynastyWarRoomPanel({ leagueId }: { leagueId: string }) {
  ...
  if (loading) {
    return (
      ...
      >
        <Loader2 className="h-4 w-4 animate-spin text-violet-300" /> Loading AF War Room — Dynasty…
      </div>
    )
  }
  if (error || !context) {
    return (
      ...
      >
        {error ?? 'AF War Room — Dynasty is unavailable for this league.'}
      </div>
    )
  }
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2">
        ...
        <h2 className="text-sm font-bold text-white">AF War Room — Dynasty</h2>
      ...
      <div className="rounded-lg border border-white/[0.06] bg-[#07071a] p-3">
        <p className="mb-1.5 flex items-center gap-1.5 text-[11px] font-bold uppercase tracking-wide text-white/40">
          <ShieldQuestion className="h-3.5 w-3.5" /> Ask AF War Room
        </p>
        <textarea
          value={question}
