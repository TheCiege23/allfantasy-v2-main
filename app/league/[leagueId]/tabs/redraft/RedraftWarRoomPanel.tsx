...
export function RedraftWarRoomPanel({ leagueId }: { leagueId: string }) {
  ...
  if (loading) {
    return (
      ...
      >
        <Loader2 className="h-4 w-4 animate-spin text-violet-300" /> Loading AF War Room — Redraft…
      </div>
    )
  }
  if (error || !context) {
    return (
      ...
      >
        {error ?? 'AF War Room — Redraft is unavailable for this league.'}
      </div>
    )
  }
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2">
        ...
        <h2 className="text-sm font-bold text-white">AF War Room — Redraft</h2>
