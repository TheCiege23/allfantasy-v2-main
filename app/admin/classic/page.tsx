...
function ProviderTeamReconciliationPanel({
  ...
  data: { summaries: ProviderTeamReconciliationSummary[]; totalProblems: number; generatedAt: string; unavailable: boolean }
}) {
  const { summaries, totalProblems, unavailable } = data
  ...
  return (
    ...
    >
      ...
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
        <MetricCard item={{ label: "Sport/provider pairs", value: summaries.length, tracked: !unavailable }} />
        <MetricCard
          item={
            unavailable
              ? { label: "True problems", value: "Unavailable", tracked: false, note: "Reconciliation query failed — not a real zero" }
              : { label: "True problems", value: totalProblems, tracked: true, note: "Ambiguous + unmapped + duplicate after fuzzy match" }
          }
        />
        <MetricCard item={{ label: "Ready pairs", value: readyCount, tracked: !unavailable, note: "≥98% covered, no ambiguous/duplicate" }} />
        <MetricCard item={{ label: "Partial pairs", value: partialCount, tracked: !unavailable, note: "≥85% covered" }} />
        <MetricCard item={{ label: "Critical pairs", value: criticalCount, tracked: !unavailable, note: "<85% covered" }} />
      </div>

      {unavailable ? (
        <p className="mt-4 rounded-xl border border-amber-400/25 bg-amber-400/[0.06] p-4 text-sm text-amber-100">
          Reconciliation query failed — this is not evidence of a clean state. Retry or check server logs.
        </p>
      ) : summaries.length === 0 ? (
        <p
...
export default async function AdminPage({
  ...
  if (gate.status === "unauthenticated") {
    redirect("/admin-login?next=/admin/classic")
  }
