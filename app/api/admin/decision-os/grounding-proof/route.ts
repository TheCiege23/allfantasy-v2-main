...
export async function GET(request: NextRequest) {
   ...
   * router below maps three of the four opt-in slices to a hard `false` — they are over the chat
   * route's latency ceiling — so mirroring the router alone would make those slices unreachable
   * from the one tool built to look at them. That is the wrong kind of faithful: the point of a
   * proof surface is to see what a slice DOES, including a slice a chat turn cannot afford.
   *
