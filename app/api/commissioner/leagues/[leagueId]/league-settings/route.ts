...
import type { GetLeagueSettingsResponse } from '@/lib/league-settings-engine/LeagueSettingsEngineTypes';
...
/**
 * 🛑 RETIRED 2026-09-24. This PUT replaced the league's ENTIRE `settings` JSON: whenever the stored
 * settings lacked this engine's `meta` block — true of every league the create flow makes — it
 * started from sport defaults, merged the request in, and wrote the result back, erasing scoring
 * configs, the invite code, keeper and playoff configuration and everything else keyed there. Co-
 * commissioners could call it. Its only client (`LeagueSettingsModalShell`) is mounted nowhere.
 *
