...
export default async function NbaPlayoffPoolsHubPage() {
  const session = (await getServerSession(authOptions as never)) as { user?: SessionUser } | null
  const userId = session?.user?.id ?? null

  const nbaPools = userId
    ? (await listUserPlayoffChallenges(userId)).filter((c) => String(c.sport).toLowerCase() === "nba")
    : []

  return (
    <main className="relative min-h-screen bg-[#05070b] text-white">
      <div className="pointer-events-none absolute inset-x-0 top-0 h-64 bg-[radial-gradient(ellipse_70%_50%_at_50%_-10%,rgba(167,139,250,0.11),transparent)]" />
      <div className="relative mx-auto max-w-[min(100%,920px)] px-4 py-8 sm:px-6 lg:px-10">
        ...
        <div className="mt-8 flex flex-col gap-6 sm:flex-row sm:items-start sm:justify-between">
          <div>
            <p className="text-[11px] font-black uppercase tracking-[0.2em] text-violet-200/70">NBA</p>
            <h1 className="mt-2 text-3xl font-black tracking-tight sm:text-4xl">NBA Playoff Pools</h1>
          ...
          <div className="flex shrink-0 flex-wrap gap-2">
            <Link
              href="/brackets/nba/create"
              className="inline-flex min-h-[44px] items-center justify-center gap-2 rounded-xl bg-violet-400 px-4 py-2.5 text-sm font-black text-black shadow-lg shadow-violet-500/20"
            >
            ...
            <Link
              ...
              className="inline-flex min-h-[44px] items-center justify-center gap-2 rounded-xl border border-violet-400/30 bg-violet-400/10 px-4 py-2.5 text-sm font-bold text-violet-100 hover:bg-violet-400/15"
            >
        ...
        <section className="mt-12 rounded-2xl border border-white/10 bg-white/[0.04] p-4 sm:p-6">
          <div className="flex items-center gap-2 text-white/90">
            ...
            <h2 className="text-sm font-black uppercase tracking-wide">My NBA pools</h2>
          </div>
          {userId ? null : (
            <p className="mt-3 text-sm text-white/45">Sign in to see playoff pools you own or joined.</p>
          )}
          {userId && nbaPools.length === 0 ? (
            <p className="mt-3 text-sm text-white/55">
              You don&apos;t have any NBA playoff pools yet.{" "}
              <Link href="/brackets/nba/create" className="font-semibold text-violet-300 hover:text-violet-200 underline-offset-4 hover:underline">
                Create one
              </Link>
              .
            </p>
              ...
              {nbaPools.map((pool) => (
                <li key={pool.challengeId}>
                  <Link
                    ...
                    className="flex flex-col rounded-xl border border-white/10 bg-black/25 px-4 py-3 transition hover:border-violet-400/35 hover:bg-white/[0.06]"
                  >
