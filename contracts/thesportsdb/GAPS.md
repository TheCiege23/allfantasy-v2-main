...
| `R-18` | `lookupplayerstats` returns **full defensive categories for NFL** | Micah Parsons (34167758) → 147 rows, seasons 2021-2025. 2024 line: Solo Tackles 30, Assisted Tackles 13, Combined Tackles 43, Sacks 12.0, Tackles For Loss 12, Quarterback Hits 23, Passes Defended 1, Forced Fumbles 2, Defensive Interceptions 0, Safeties 0, Games Played 13. So `G-04`'s defensive half is answered: this provider DOES carry IDP production. Still **season aggregates only** — no weekly lines (`R-04`). Fixture: `v1.lookupplayerstats.NFL_defender.json`. |
| `R-19` | ⚠️ **`Total Tackles` MEANS SOLO, NOT TOTAL** | Same fixture, 2024: `Total Tackles` 30 and `Solo Tackles` 30 are the same number, while `Combined Tackles` 43 = 30 solo + 13 assisted. A parser trusting the name undercounts every defender by his assists — for a tackle-heavy IDP league that is the single biggest scoring category. Use `Combined Tackles`, or `Solo Tackles` + `Assisted Tackles`. Near-duplicate pairs exist elsewhere too (`Sacks`/`Tackle Sacks`, `Pass Defended`/`Passes Defended`, `Tackle For a loss`/`Tackles For Loss`). |
...
| `G-04` | `lookupplayerstats` position coverage | **DEFENSIVE RESOLVED 2026-08-27** | player profiles | QB returned passing + rushing. **Defenders return a full defensive line — see `R-18`.** Still untested: receiving (WR/TE), kicking. |
