...
export function WarRoomMiniCard({
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok || !json.ok) {
        setData(null)
        setError((json as { error?: string }).error || 'Could not load AF War Room')
        return
      }
      setData(json)
    } catch {
    ...
    >
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        <button
          ...
          aria-label="Refresh AF War Room"
        >
      ...
      <div className="mt-3 min-h-[52px]">
        {loading && !data ? (
          ...
          <p className="text-[12px] text-[#8b93ab]">Select a league to load AF War Room.</p>
        )}
      </div>
