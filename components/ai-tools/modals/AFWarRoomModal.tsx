...
export function AFWarRoomModal({
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (!r.ok || !json.ok) {
        setData(null)
        setError((json as { error?: string }).error || 'AF War Room could not load.')
        return
      }
      setData(json)
    } catch {
  ...
  const chimmyHref = useMemo(() => {
    ...
    const prompt = data?.aiSummary
      ? `AF War Room summary (facts only):\n${data.aiSummary}\n\nHelp me prioritize the next roster moves.`
      : 'AF War Room: what should I do first with my synced league data?'
  ...
  return (
    <AIToolModalShell
      ...
      emptyMessage="AF War Room returned no prioritized actions for this scope — try enabling more modules or picking a specific league."
      headerBadge={headerBadge}
      onRefresh={() => void load()}
      refreshing={loading}
      chimmyPrompt="AF War Room: synthesize my prioritized actions from live AllFantasy modules."
    ...
    return (
      <div className="space-y-3">
        ...
        >
          ...
          Continue in Chimmy with AF War Room context
