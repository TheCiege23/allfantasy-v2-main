...
/**
 * components/league-settings/NcaabScoringSettingsPanel.tsx
 * NCAAB (College Basketball) Commissioner Scoring Settings — full tabbed UI.
 * 10 category tabs mirroring the NBA scoring panel structure:
 * General, Shooting, Free Throws, Three-Point, Rebounds,
 * Playmaking, Defense, Discipline, Bonuses, Advanced
 *
 * College-specific adjustments: lower bonus thresholds, tempo-adjusted
 * advanced hooks, personal foul defaults.
 *
 * ONE NCAAB LEAGUE = ONE SCORING SYSTEM
 * The same scoring config is used across ALL NCAAB league types (Redraft,
 * Dynasty, Keeper, Best Ball, Tournament/March Madness, Guillotine, Survivor,
 * Zombie, Big Brother, Devy, C2C, etc.). Specialty league mechanics are separate.
 */
...
import {
  ...
  type NcaabScoringCategory,
  type NcaabScoringRow,
} from '@/lib/ncaab-scoring/NcaabScoringCategories'

// ---------------------------------------------------------------------------
// Types
// ---------------------------------------------------------------------------

type PresetKey = 'af_default' | 'sleeper_compatible' | 'espn_compatible' | 'yahoo_compatible' | 'custom'

interface NcaabScoringPreset {
  key: PresetKey
  label: string
  description: string
  warning?: string
...
export function NcaabScoringSettingsPanel({ leagueId, isCommissioner = false }: Props) {
  ...
  const allCategories: NcaabScoringCategory[] = useMemo(
    () => [...NCAAB_SCORING_CATEGORIES, NCAAB_PREMIUM_SCORING],
    [],
  )
  ...
  useEffect(() => {
    let active = true
    fetch(`/api/commissioner/leagues/${encodeURIComponent(leagueId)}/ncaab-scoring`, { cache: 'no-store' })
      .then((r) => r.json())
      .then((data) => {
        if (!active) return
  ...
  const handleRuleChange = useCallback((statKey: string, value: number) => {
    ...
    if (!isPremium) {
      const isPremiumRow = NCAAB_PREMIUM_SCORING.rows.some((r) => r.key === statKey)
  ...
  const save = useCallback(async () => {
    ...
    try {
      const res = await fetch(`/api/commissioner/leagues/${encodeURIComponent(leagueId)}/ncaab-scoring`, {
        method: 'PUT',
    ...
    <div className="space-y-5">
      ...
      {activeCategory && (
        <div className="max-h-[52vh] space-y-0.5 overflow-y-auto rounded-xl border border-white/10 bg-white/[0.02] p-3">
          ...
          {/* Stat rows */}
          {activeCategory.rows.map((row: NcaabScoringRow) => {
