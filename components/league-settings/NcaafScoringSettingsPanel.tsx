...
/**
 * components/league-settings/NcaafScoringSettingsPanel.tsx
 * NCAAF Commissioner Scoring Settings — full tabbed panel mirroring the NFL UI.
 *
 * Tabs: Passing | Rushing | Receiving | Kicking | Team Defense |
 *       Special Teams | Misc | Bonus | IDP | Advanced (premium)
 *
 * ONE scoring config per league — applies to ALL NCAAF league types:
 *   Redraft, Dynasty, Keeper, Best Ball, Auction, Guillotine,
 *   Survivor, Zombie, Tournament, Big Brother, Devy, C2C, IDP, Superflex, TE Premium.
 *
 * League mechanics (Guillotine elimination, Zombie infection, Tournament rounds,
 * Devy player rights, C2C dual-track) do NOT alter the scoring config.
 *
 * Commissioner = editable. Non-commissioner = read-only.
 * Preset selector, scrollable stat rows, helper text, Save/Reset controls.
 * Advanced tab gated behind AF Commissioner Subscription.
 *
 * C2C Note: When sport === 'NCAAF' with C2C format, this panel controls the
 * college-side scoring. The NFL scoring panel controls the pro-side scoring.
 * Both configs coexist under league.settings.ncaaf_scoring_config and
 * league.settings.nfl_scoring_config respectively.
 */
...
import {
  ...
  buildNcaafScoringDefaults,
  type NcaafScoringCategory,
  type NcaafScoringRow,
} from '@/lib/ncaaf-scoring/NcaafScoringCategories'
import type { NcaafScoringPresetKey } from '@/lib/ncaaf-scoring/NcaafScoringPresets'

// ---------------------------------------------------------------------------
// Types
// ---------------------------------------------------------------------------

interface NcaafScoringPreset {
...
interface NcaafScoringConfig {
  presetKey: NcaafScoringPresetKey
  presetLabel: string
  source: string
  rules: Record<string, number>
...
// ---------------------------------------------------------------------------
// Tab colour map — mirrors NFL panel colours exactly
// ---------------------------------------------------------------------------
...
export function NcaafScoringSettingsPanel({ leagueId, isCommissioner = false }: Props) {
  ...
  const [selectedPreset, setSelectedPreset] = useState<NcaafScoringPresetKey>('af_default')
  const [editedRules, setEditedRules] = useState<Record<string, number>>({})
  const [activeTab, setActiveTab] = useState<string>('passing')

  // All categories: 9 standard + premium Advanced tab
  const allCategories: NcaafScoringCategory[] = useMemo(
    () => [...NCAAF_SCORING_CATEGORIES, NCAAF_PREMIUM_SCORING],
    [],
  )
  ...
  useEffect(() => {
    let active = true
    fetch(`/api/commissioner/leagues/${encodeURIComponent(leagueId)}/ncaaf-scoring`, { cache: 'no-store' })
      .then((r) => r.json())
      .then((data) => {
        if (!active) return
        ...
        setSelectedPreset((data.config?.presetKey ?? 'af_default') as NcaafScoringPresetKey)
        // Merge category defaults with saved rules so every key has a value
  ...
  const hasChanges = useMemo(() => {
    ...
    const savedMerged = { ...buildNcaafScoringDefaults(), ...config.rules }
    return JSON.stringify(editedRules) !== JSON.stringify(savedMerged)
  }, [editedRules, config])
  ...
  const handleRuleChange = useCallback(
    (statKey: string, value: number) => {
      ...
      if (!isPremium) {
        const isPremiumRow = NCAAF_PREMIUM_SCORING.rows.some((r: NcaafScoringRow) => r.key === statKey)
  ...
  const save = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        `/api/commissioner/leagues/${encodeURIComponent(leagueId)}/ncaaf-scoring`,
        {
          method: 'PUT',
    ...
    <div className="space-y-5">
      ...
      <div className="space-y-2">
        ...
        <div className="flex flex-wrap gap-2">
          {(
            ['af_default', 'sleeper_compatible', 'espn_compatible', 'yahoo_compatible', 'custom'] as NcaafScoringPresetKey[]
          ).map((key) => {
      ...
      {/* Warning banner for external/compatible presets */}
      ...
      {activeCategory && (
        <div className="max-h-[52vh] space-y-0.5 overflow-y-auto rounded-xl border border-white/10 bg-white/[0.02] p-3">
          ...
          {/* Stat rows */}
          {activeCategory.rows.map((row: NcaafScoringRow) => {
